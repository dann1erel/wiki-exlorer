...
```bash
wiki-explorer-gpt --verbose <command> ...
wiki-explorer-gpt <command> ... --verbose
```
...
```bash
wiki-explorer-gpt search "искусственный интеллект" --limit 20 --lang ru
```

Поиск в английской Wikipedia:

```bash
wiki-explorer-gpt search "Python programming" --limit 10 --lang en
```

Сортировка по релевантности:

```bash
wiki-explorer-gpt search "машинное обучение" --sort relevance --limit 10 --lang ru
```

Сортировка по дате последнего изменения:

```bash
wiki-explorer-gpt search "Python" --sort last_edit --limit 10 --lang en
```

Запуск с подробным логированием:

```bash
wiki-explorer-gpt search "Python" --sort last_edit --limit 10 --lang en --verbose
```

Глобальный запуск с подробным логированием:

```bash
wiki-explorer-gpt --verbose search "Python" --sort last_edit --limit 10 --lang en
```
...
```bash
wiki-explorer-gpt images "Solar System" --limit 3 --download --output ./pictures --lang en
```
...
```bash
wiki-explorer-gpt random --lang ru
```

Случайная статья в английской Wikipedia:

```bash
wiki-explorer-gpt random --lang en
```

Случайная статья из категории:

```bash
wiki-explorer-gpt random --category "Наука" --lang ru
```

```bash
wiki-explorer-gpt random --category "Science" --lang en
```

С фильтром по примерному количеству слов:

```bash
wiki-explorer-gpt random --min-words 1000 --lang ru
```

Только статьи с изображением:

```bash
wiki-explorer-gpt random --with-image --lang ru
```

С несколькими фильтрами и подробным логированием:

```bash
wiki-explorer-gpt random --category "Science" --min-words 1000 --with-image --lang en --verbose
```

Глобальный подробный режим также поддерживается:

```bash
wiki-explorer-gpt --verbose random --category "Science" --min-words 1000 --with-image --lang en
```
...
```bash
wiki-explorer-gpt info "Python" --show-categories --show-image --lang en
```

С подробным логированием:

```bash
wiki-explorer-gpt info "Python" --show-categories --show-image --lang en --verbose
```

Глобальный подробный режим:

```bash
wiki-explorer-gpt --verbose info "Python" --show-categories --show-image --lang en
```
...
```bash
wiki-explorer-gpt links "Python" --search "programming" --limit 50 --lang en
```

С подробным логированием:

```bash
wiki-explorer-gpt links "Python" --search "programming" --limit 50 --lang en --verbose
```

Глобальный подробный режим:

```bash
wiki-explorer-gpt --verbose links "Python" --limit 50 --lang en
```
...
```bash
wiki-explorer-gpt graph "Python" --depth 2 --limit 10 --output graph.png --format png --lang en
```

Вывести одноуровневый граф в текстовом виде:

```bash
wiki-explorer-gpt graph "Python" --limit 20 --text --lang en
```

Вывести двухуровневый граф в текстовом виде:

```bash
wiki-explorer-gpt graph "Python" --depth 2 --limit 5 --text --lang en
```

С подробным логированием:

```bash
wiki-explorer-gpt graph "Python" --depth 2 --limit 5 --text --lang en --verbose
```

Глобальный подробный режим:

```bash
wiki-explorer-gpt --verbose graph "Python" --depth 2 --limit 5 --text --lang en
```
...
```bash
wiki-explorer-gpt categories "Python" --lang en
```

Пример с ограничением количества категорий:

```bash
wiki-explorer-gpt categories "Python" --limit 20 --lang en
```

Пример древовидного вывода:

```bash
wiki-explorer-gpt categories "Python" --tree --limit 10 --lang en
```

С подробным логированием:

```bash
wiki-explorer-gpt categories "Python" --tree --limit 10 --lang en --verbose
```

Глобальный подробный режим:

```bash
wiki-explorer-gpt --verbose categories "Python" --tree --limit 10 --lang en
```
...
```bash
wiki-explorer-gpt pageviews "Python"
```

Период 90 дней:

```bash
wiki-explorer-gpt pageviews "Python" --days 90
```
...
```bash
wiki-explorer-gpt pageviews "Python" --days 30 --chart png --output pageviews.png
```

Запуск с подробным логированием для команды:

```bash
wiki-explorer-gpt pageviews "Python" --days 30 --chart ascii --verbose
```

Глобальный запуск с подробным логированием:

```bash
wiki-explorer-gpt --verbose pageviews "Python" --days 30 --chart ascii
```
...
```bash
wiki-explorer-gpt images "Солнечная система" --lang ru
```

```bash
wiki-explorer-gpt images "Solar System" --limit 10 --lang en
```
...
```bash
wiki-explorer-gpt images "Solar System" --download --all --output ./pictures --lang en
```

Запуск с подробным логированием:

```bash
wiki-explorer-gpt images "Python" --limit 5 --download --output ./downloads --lang en --verbose
```

Глобальный подробный режим также поддерживается:

```bash
wiki-explorer-gpt --verbose images "Python" --limit 5 --download --output ./downloads --lang en
```
...
Через установленную CLI-команду:
...
```bash
wiki-explorer-gpt info "Python"
```

```bash
wiki-explorer-gpt links "Python"
```

```bash
wiki-explorer-gpt graph "Python" --text
```

```bash
wiki-explorer-gpt categories "Python"
```
...
```bash
wiki-explorer-gpt images "Солнечная система"
```
